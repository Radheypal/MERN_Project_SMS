...
export default function JobMatch() {
  ...
  const handleAIShortlist = async () => {
    ...
    } catch (err) { alert('AI shortlisting failed'); }
  ...
  const rankColor = { High: '#16a34a', Medium: '#d97706', Low: '#dc2626' };
      ...
      <div style={s.grid}>
        ...
        <div style={s.card}>
          ...
          <form onSubmit={handleMatch}>
            ...
            <button type="button" onClick={handleAIShortlist} style={s.aiBtn} disabled={aiLoading}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                          <span style={{ color: '#64748b', fontSize: '13px' }}>💼 {c.experience} yrs</span>
                          <span style={{ color: '#64748b', fontSize: '13px' }}>✅ Matched: {c.matchedSkills.join(', ') || 'None'}</span>
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                          {c.skills.map(skill => (
                            <span key={skill} style={{ background: c.matchedSkills.includes(skill) ? '#d1fae5' : '#f1f5f9', color: c.matchedSkills.includes(skill) ? '#065f46' : '#64748b', padding: '2px 8px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>
                              {skill}
                            </span>
                          ))}
                        </div>
...
const s = {
  ...
  rank: { background: '#1e1b4b', color: '#fff', width: '26px', height: '26px', borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '800' },
  qBtn: { background: '#ede9fe', color: '#6d28d9', border: 'none', padding: '8px 14px', borderRadius: '8px', fontWeight: '600', fontSize: '12px', cursor: 'pointer', marginLeft: '10px', whiteSpace: 'nowrap' },
